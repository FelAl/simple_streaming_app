...
2. Как подсчитать максимальную и минимальную задержку между появление сообщения в кафка producer и появлением результата обработки этого сообщения в нашем приложении(терминал, база, распределенная память)?
Для минимальной задержки - отправить одно сообщение, зафиксировать время, и в спарке сделать вывод времени когда она было обработано. Часы должны быть синхронизированы и показывать точное время. Так проделать раз 5-6, и получить среднее значение на основе пяти опытов.

Для максимальной задержки - начать отправлять очень много сообщений, нужно найти такое количество сообщений в единицу времени, чтобы часть из них или хотя бы одно перестало обрабатываться. Затем,
это количество сообщений начинаем снижать, чтобы начало обрабатываться 100% сообщений. Фиксируем время отправки последнего сообщения и фиксируем время его обработки в спарке несколько раз. Это будет максимальная задержка
...
В случае со scala `addShutdownHook` и поместить туда необходимый код.
Но в документации написано `Note that shutdown hooks are NOT guaranteed to be run.`(http://www.scala-lang.org/files/archive/api/current/#scala.sys.package)
